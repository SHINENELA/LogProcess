...
class Match:
    ...
    def drawTree(self):
        ...
        if True:
            ...
            for pid in self.tree.tree_list:
                ...
                A.write('./output/refooOld.dot')
                A.layout('dot') # layout with dot
                A.draw('./output/reBuildTree.png') # write to file
        ...
        if match_model == 4:
            ...
            for word in log_words:
                    if word in self.words_frequency:
                        words_index[word] = self.words_frequency.index(word)  #在fre文件中的行数
                    # else:
    ...
    def matchLogsFromFile(self, para):
        ...
        out_seq_path = para['out_seq_path_ordered']
        ...
        print('# of unmatched log (except ):', count_zero)
    ...
    def matchLogsAndLearnTemplateOneByOne(self, para):
        ...
        short_threshold = para['short_threshold']
        match_model = para['match_model']
        out_seq_path_new = para['out_seq_path_new']

        f = open(out_seq_path_new, 'w')
        short_log = 0
        # short_threshold = 5
        count_zero = 0
        total_num = 0
        ...
        with open(new_logs_path) as IN:
            for line in IN:
                ...
                tag, cur_match = self.match(log_words, match_model)
                # print (line.strip())
                print ('tag:',tag,'~~cur_match:',cur_match)
                # print ('')
                if len(log_words)< short_threshold:#过滤长度小于5的日志
                    short_log+=1
                    tag = -1
...
#                    self.drawTree()
                    ...
                    ff.writelines(' '+cur_match+'\n')
                    ff.close()
#匹配到了输出1~n，没匹配到输出新增量学习的模板号，日志小于过滤长度输出-1
                f.writelines(str(tag)+'  '+str(timestamp)+'\n')

        print ('filting # short logs:',short_log,'| threshold =',short_threshold)
        print ('# of unmatched log(new template?) (except filting):', count_zero)
        print ('# of total logs:',total_num)
        print ('seq_file_path:',out_seq_path_new)
...
def match(para):
    ...
    if para['match_model'] == 1:
        ...
        para = {
            ...
            'template_path' : args.order_templates,
            ...
            'log_path': args.new_logs_path,
            'out_seq_path_ordered': args.out_seq_path_ordered
        }
        #print('@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@print_help()&&&&&&&&&&&&&&&&&&&&&&&')
        #parser.print_help()
        mt1 = Match(para)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-template_path', help='plot_flag', type=str, default="./output/output.template")
    parser.add_argument('-fre_word_path', help='fre_word_path', type=str, default="./output/output.fre")
    parser.add_argument('-runtime_log_path', help='log_path', type=str, default='./training.log')
    parser.add_argument('-out_seq_path', help='out_seq_path', type=str, default='./output/output.seq')
    parser.add_argument('-plot_flag', help='画图, 如树太大不要画图，会卡死', type=int, default=1)
    parser.add_argument('-CUTTING_PERCENT', help='增量学习时会用到，正常匹配用不到',type=float, default = 0.6)
    parser.add_argument('-NO_CUTTING', help='增量学习时会用到，正常匹配用不到', type=int, default=1)#初步设定1时，是前60% 不剪枝 ,全局开关， 当其为0时，全局按照min_threshold剪枝
    parser.add_argument('-match_model', help='1:正常匹配  2:单条增量学习&匹配 3:批量增量学习&匹配 4:正序匹配', type=int, default = 4)
    parser.add_argument('-new_logs_path',type=str,default='./new.log')
    parser.add_argument('-out_seq_path_new',type=str,default='./output/output.learnonebyone.seq')
    parser.add_argument('-out_seq_path_ordered', type=str, default='./output/order/output.ordered.seq')
    parser.add_argument('-order_templates', type=str, default='./output/order/output.order.template')
    ...
    para = {
        ...
        'match_model' : args.match_model,
        'log_path' : args.new_logs_path,
        'out_seq_path_new': args.out_seq_path_new,
        'out_seq_path_ordered': args.out_seq_path_ordered
    }
