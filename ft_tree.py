...
# * Description   :
'''
pids in this realization are ignore, so all lines with pids can be ignore
'''
# **********************************************************
...
class WordsFrequencyTree(object):
    ...
    def do(self, logs, para):
        ...
        for pid in self.tree_list:
            ...
            print('path1:', self.tree_list[pid])
        ...
        for pid in all_paths:
            ...
            for path in all_paths[pid]:
                ...
                # print (i, pid, end=' ')
                # 首先把pid保存下来
                f.write(pid + " ") #不保存index  这行是源代码中存在的
    ...
    def drawTree(self):
        ...
        for pid in self.tree_list:
            ...
            A.write('./output/fooOld.dot')
            A.layout('dot') # layout with dot
            A.draw('./output/Trace.png') # write to file
...
def getLogsAndSave(para):
    ...
    print ('template_path:', output_name)  #模板 short\_threshold = 5 #过滤掉长度小于5的日志
...
if __name__ == "__main__":
    ...
    parser.add_argument('-template_path', type=str, default='./output/output.template')
    parser.add_argument('-fre_word_path', type=str, default='./output/output.fre')
    parser.add_argument('-picture_path', type=str, default='./tree.png')
    parser.add_argument('-leaf_num', type=int, default=4)
    parser.add_argument('-short_threshold', type=int, default=1)#过滤掉长度小于5的日志
    parser.add_argument('-plot_flag', help='画图, 如树太大不要画图，会卡死', type=int, default=1)#如果要画图 则为1
