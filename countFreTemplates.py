...
#保存Top 3的模板 并且输出每条模板对应的日志文件
top3_temp_dir={}# {tag:template}
temp_list=[]
with open(templatePath) as IN:
    for line in IN:                 
        temp_list.append(line.strip())

print ('top3 templates:')
for i in range(3):
    tag=sorted_final_tuple[i][0]
    top3_temp_dir[tag]=temp_list[int(tag) - 1]
    print (tag, countDir[tag], top3_temp_dir[tag])#输出前10位的模板标签、出现次数和日志模板
print ('')
index=0
...
with open(logSeqPath) as IN:
    for line in IN:
        l=line.strip().split()
        tag=l[0]
        if tag in top3_temp_dir:
            save_dir[index]=tag
        index+=1
#tag_log={}#{tag:rawlog_list[]}
f_dir={}#{tag:file_iter}

rank=out_path+'top3_templates.txt'
#文件中包含前3名模板标签、出现次数和对应的模板，没有出现匹配模板的所有原始日志
index=0
temp_list=[]
...
with open(new_log_path) as IN:
    for log in IN:
        ...
        if index in save_dir:
            tag=save_dir[index]
            cur_template=top3_temp_dir[tag]
            content = out_paths + str(tag) + '.txt'
